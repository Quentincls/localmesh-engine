# Le post-traitement d o_voxel, SANS nvdiffrast.
#
# La wheel o_voxel telechargee rasterise l atlas UV avec nvdiffrast, qui est
# sous NVIDIA Source Code License : usage NON COMMERCIAL. LocalMesh est vendu
# 39 euros. Ce fichier est la copie de son postprocess.py dont le seul bloc
# de rasterisation a ete remplace par uvraster, le rasteriseur PyTorch ecrit
# dans ce projet — substitution verifiee, 99,7 % des texels partages, erreur
# maximale 0,0014.
#
# POURQUOI IL VIT ICI ET PLUS DANS site-packages. Le correctif etait applique
# a la main sur la copie installee : invisible du depot, perdu a la premiere
# reinstallation, et impossible a relire pour savoir ce qui avait ete change.
# `patches.py` remplace desormais o_voxel.postprocess.to_glb par celui-ci au
# chargement, donc la wheel d origine peut rester telle quelle sur le disque.
#
# Ce fichier est du code tiers modifie : le reprendre tel quel a chaque montee
# de version d o_voxel, en refaisant la seule substitution ci-dessous.
...
def to_glb(
    ...
    # Rasterisation de l'atlas UV sans nvdiffrast (licence NVIDIA non
    # commerciale). Le decoupage par paquets de 100 000 faces n'existait
    # que pour contourner le plafond de faces par appel de nvdiffrast :
    # uvraster n'a pas cette limite, un seul appel suffit.
    (pos,), mask = uvraster.rasterise(
        out_uvs, out_faces.to(torch.int64), texture_size, [out_vertices],
    )
    valid_pos = pos[mask]
