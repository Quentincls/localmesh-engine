...
# ProgressBar (ComfyUI) retire : `pbar` est un parametre optionnel,
# l'appelant passe le sien ou None.
...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def from_pretrained(cls, path: str, config_file: str = "pipeline.json", keep_models_loaded = True, use_fp8 = False, use_reconviagen = False, isPixal3D = False) -> "Trellis2ImageTo3DPipeline":
        ...
        if not isPixal3D:
            pipeline._pretrained_args['models']['sparse_structure_decoder'] = os.path.join(str(contexte.modeles()),"microsoft","TRELLIS-image-large","ckpts","ss_dec_conv3d_16l8_fp16")
            
        facebook_model_path = os.path.join(str(contexte.modeles()),"facebook","dinov3-vitl16-pretrain-lvd1689m")
    ...
    def load_moge_model(self):
        ...
        moge_model_path = os.path.join(str(contexte.modeles()), "Ruicheng","moge-2-vitl")
    ...
    def sample_shape_slat_cascade_multiview(
        ...
        slat = sampler_lr.sample(
            ...
            **sampler_params_combined,
            ...
            front_axis=front_axis,
            blend_temperature=blend_temperature,
            grille=sparse_structure_resolution,
            verbose=verbose,
            dino_lock=dino_lock,
            dino_substeps=dino_substeps,
            dino_foundation_cap=dino_foundation_cap,
    ...
    def postprocess_mesh(
        ...
        # Rasterisation de l'atlas UV sans nvdiffrast, dont la licence NVIDIA
        # interdit de vendre quoi que ce soit qui s'en sert. Ces lignes ne
        # demandaient qu'une chose : pour chaque texel, la position du modele
        # qui lui correspond, et la couverture. C'est la signature exacte de
        # uvraster.rasterise, qui prend l'UV dans [0,1] et rend deja l'ordre
        # bas-haut de nvdiffrast.
        
        torch.cuda.synchronize()
        
        (pos,), mask = uvraster.rasterise(
            uvs_torch, faces_torch.to(torch.int64), texture_size, [vertices_torch],
        )
        ...
        # LocalMesh 3.2 (6 septembre 2026) : le canal métal de TRELLIS.2 est
        # du bruit (0,6 à 0,8 en moyenne sur un samouraï en résine) ; dans
        # toute visionneuse il assombrit la couleur et teinte les reflets,
        # ce qui « sature » l'objet. Il est écrit à zéro.
        metallicRoughnessTexture = Image.fromarray(np.concatenate([np.zeros_like(metallic), roughness, np.zeros_like(metallic)], axis=-1))
    ...
    def texture_mesh(
        ...
        if resolution==512:            
            ...
            if not self.keep_models_loaded:
                # ECART A L AMONT (LocalMesh, 2 septembre 2026). L original
                # dechargeait ici `shape_slat_flow_model_1024` — un modele que
                # `run()` a deja rendu — au lieu du DiT de TEXTURE 1024 qui
                # vient de servir. La branche 512 juste au-dessus fait la bonne
                # chose. Consequence sur le chemin Standard / Detaille : le DiT
                # de texture (1,3 Go) n etait jamais retire de `self.models`,
                # et restait a demeure sous le pic de cuisson de l atlas et de
                # la generation suivante. C est precisement l etape qui deborde
                # sur une carte de 8 Go.
                self.unload_tex_slat_flow_model_1024()
