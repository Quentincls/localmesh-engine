...
# meshlib N EST PLUS LA DU TOUT. Sa licence est un « NON-COMMERCIAL &
# education LICENSE AGREEMENT » (AMV Consulting) : usage non commercial,
# evaluation ou enseignement. LocalMesh est vendu, et ce depot est public.
#
# Importe au niveau module, il etait charge par CHAQUE generation — le simple
# acces a la classe Mesh suffisait, verifie a chaud. La seule fonction qui s en
# servait n etait appelee par personne : la decimation passe par cumesh. Elle
# a donc ete supprimee, et pas seulement mise en sommeil.
#
# Meme forme que le probleme nvdiffrast, trouve le meme jour.
...
class Mesh:
    ...
    # LA DECIMATION PAR MESHLIB A ETE RETIREE.
    #
    # Elle n'etait appelee par personne : la decimation du moteur passe par
    # cumesh. Sa licence, elle, est un accord « non commercial, evaluation ou
    # enseignement » (AMV Consulting), incompatible avec ce depot comme avec
    # le produit qui l'utilise. Une methode qu'on ne peut pas appeler sans
    # enfreindre une licence n'a rien a faire dans un depot public.
